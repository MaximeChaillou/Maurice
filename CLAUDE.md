...
- **Code reuse**: Maximize reuse of existing code. Before creating a new function, check if a similar implementation already exists in the project. Extract common patterns into shared methods/extensions rather than duplicating code. Never write the same logic in two places — if the same data (list of templates, configuration values, etc.) is needed in multiple call sites, centralize it in a single source of truth and call that from each site.
- **Generic reusable components**: Never duplicate UI or behavior across screens. When two views need the same widget (sidebar row, doc header, breadcrumb, action pill, sheet, helper, etc.), build a single generic component parameterized by its inputs (data, callbacks, optional `@ViewBuilder` slots, `Binding`s for two-way state) and use it in both places. If the same code appears in two screens — even if "they happen to need the same thing" — extract it. Pull state up to the lowest common ancestor when needed so the component can be truly shared. The same goes for ViewModel logic: extract free helpers / static methods rather than repeating the same matching/formatting/lookup in multiple view models.
- **Shared components**: Reuse the shared building blocks before writing new ones:
  - Tab screen layout: `TabScreenLayout`, `GlassSidebar`, `SidebarRow`, `SidebarSectionLabel`, `SidebarEmptyState`, `TabDetailScaffold`, `TabContentCard` (in `SharedTabComponents.swift`).
  - Doc header: `TabDocHeader` (in `TabDocHeader.swift`) — supports editable icon (emoji picker), editable title, status pill, meta items, and a `trailingActions` slot. Use `TabHeaderIcon`, `TabMetaItem`, `StatusPill`.
  - Breadcrumb: `BreadcrumbBar` with `BreadcrumbSegment` / `BreadcrumbSibling` / `BreadcrumbSiblingGroup` (in `BreadcrumbBar.swift`) — clickable segments with sibling popovers.
  - Action pills: `TabPillButton`, `TabSquareButton`, `TranscriptPill`, `EntryMoreMenu`, `QuickNotesPillButton`, `SkillsPillMenu` (in `TabActionButtons.swift`).
  - Schedulable folder actions: `MeetingActionsBar` (in `MeetingActionsBar.swift`) — encapsulates the QuickNotes + Skills pills plus their config / add-action sheets for any folder backed by a `MeetingConfig` (works for meeting folders and 1-1 folders).
  - Calendar lookup: `LinkedEventInfo.findUpcomingEvent`, `LinkedEventInfo.statusLabel`, `LinkedEventInfo.timeLabel`, `SidebarDateFormatter.upcomingLabel`, `SidebarDateFormatter.relativeLabel` (in `Domain/Entities/`).
  - Date entries: `DateEntryContentView`, `.deletionAlert()`, `.entryDeleteAlert()` modifiers (in `DateNavigationComponents.swift`).
  - Avatars: `AvatarColors.initials(for:)`, `AvatarColors.gradient(for:)`.
  - Markdown editing: `FolderFileEditorView` / `FolderFileDetailView` with `FolderFile(url:)` — do not recreate load/save logic.
